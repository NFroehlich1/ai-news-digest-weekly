...
class DecoderService {
  ...
  async generateSummary(digest: WeeklyDigest, selectedArticles?: RssItem[], linkedInPage?: string): Promise<string> {
    try {
      ...
      // Use selected articles if provided, otherwise use ALL from digest
      const items = selectedArticles || digest.items;
      
      // Ensure each article is distinct by using guid or link as identifier
      const uniqueItems = this.getUniqueArticles(items);
      console.log(`Using ${uniqueItems.length} unique articles from ${items.length} total articles`);
      
      // Get titles and descriptions to create a prompt
      const contentSummaries = uniqueItems.map(item => 
        `Titel: ${item.title}\n${item.aiSummary ? 'AI-Zusammenfassung: ' + item.aiSummary : 'Beschreibung: ' + (item.description ? item.description.substring(0, 500) + '...' : 'Keine Beschreibung verfügbar')}\nURL: ${item.link}\n`
      ).join("\n");
      ...
      basierend auf diesen ${uniqueItems.length} AI-News-Artikeln:
      ...
      - Detaillierte Zusammenfassungen von ALLEN ${uniqueItems.length} Nachrichtenartikeln mit:
      ...
      console.log(`Generating comprehensive newsletter for digest (${uniqueItems.length} articles):`, digest.id);
      ...
      if (!response.ok) {
        ...
        return this.formatComprehensiveNewsletter(digest, uniqueItems, linkedInPage);
      }
  ...
  // Helper method to ensure articles are unique (no duplicates)
  private getUniqueArticles(items: RssItem[]): RssItem[] {
    // Use a Map to track unique articles by guid or link
    const uniqueArticlesMap = new Map<string, RssItem>();
    
    for (const item of items) {
      // Use guid as primary identifier, or link as fallback
      const identifier = item.guid || item.link;
      if (!uniqueArticlesMap.has(identifier)) {
        uniqueArticlesMap.set(identifier, item);
      }
    }
...
${uniqueItems.map(item => `- **${item.title.split(':')[0]}**: ${item.aiSummary ? item.aiSummary.split('.')[0] + '.' : 'Wichtige Entwicklung im KI-Bereich.'}`).join('\n')}
